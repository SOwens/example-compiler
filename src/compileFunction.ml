...
let compile_fun (f : func) : id * X86.instruction list =
  ...
  let lva_cfg0 = LiveVarAnalysis.lva cfg' in
  ...
  *)
  ...
  let lva_cfg2 = LiveVarAnalysis.lva (List.map fst lva_cfg1) in
  ...
  let lva_cfg4 = LiveVarAnalysis.lva (List.map fst lva_cfg3) in
  ...
  (f.fun_name, InstrSelX86.to_x86 linear num_stack)
